...
module.exports = async function fetchSubmittedLogs(date) {
  // async function fetchSubmittedLogs(date) {
  ...
  try {
    ...
    // console.log('🚀 ~ file: fetchSubmittedLogs.js:123 ~ fetchSubmittedLogs ~ response:', response.data.data)
    let mins = 0;
    response.data.data.forEach(log => {
      mins += log.hoursInMins;
    })
    if (mins < Number(process.env.HOURS_PER_DAY) * 60) {
      console.log(`Less than ${process.env.HOURS_PER_DAY} hours on date: ${date} mins: ${mins} hours: ${mins / 60}`)
    }
    return response;
  } catch (error) {
